...
**Show cost** (`showCost`, default `false`): whether the subagent surfaces print an estimated cost beside their token counts — the widget (running *and* finished lines), [FleetView](#fleetview), foreground results, `get_subagent_result`, and completion notifications. (The conversation viewer's header does not carry it: it shows identity and context pressure instead — see [Conversation viewer](#ui).)
...
**Viewer markdown** (`viewerMarkdown`, default `"all"`): how much of the [conversation viewer](#ui)'s transcript is rendered as Markdown rather than shown verbatim.

```text
off        every line literal
assistant  assistant text rendered; block bodies shown verbatim
all        an expanded block's body rendered as Markdown too      (default)
```

Scoped rather than all-or-nothing because the two kinds of content have different contracts. Assistant text *is* Markdown — the model writes it that way, and the viewer was the only surface showing its source. A tool result is whatever bytes the tool produced, and a Markdown pass over one rewrites things that occur constantly in real output: `# section` in a shell script loses its `#`, a `---` line is swallowed as a setext heading, indented output is re-fenced, and `| a | b |` is redrawn as a box-drawing table. Each of those reads as the *tool* having misbehaved, which is why `all` is opt-in. A block body is folded until you press `e`, so `all` has something to render only on an expanded body — expansion is what reveals the text the setting applies to.
...
Markdown is the default because it is what the reader wants most of the time; turn it down to `assistant` or `off` for a tool whose output is data rather than prose. The viewer used to cycle this with an `m` key, which is gone — every viewer renders Markdown, and this setting (in `/agents → Settings`) is the only switch left. Code fences are syntax-highlighted using pi's own Markdown theme. Block bodies are not dimmed at all: the indent and the head's `⟳`/`✔`/`✘` mark carry the hierarchy, and the body is the part you opened the view to read. Applied live; also settable from `/agents → Settings → Viewer markdown`.
...
src/                  # Layered by domain. A directory may import only from the layers below
                      # it: `lib/` imports nothing internal, and nothing imports `ui/`, `tools/`
                      # or `extension/` except `index.ts`. Enforced mechanically by the
                      # acyclicity and layering check described in this repo's plan notebook.
  index.ts            # Extension entry: registration and wiring only. Its PATH is
                      # load-bearing — `index.ts` directly inside `src/` canonicalises to the
                      # allowlist token `src`, which agent frontmatter writes as
                      # `extensions: [src]`, `exclude_extensions: [src]` and `tools: ext:src`.
                      # Do not move it; `test/entry-manifest.test.ts` guards that.
  ...
  ui/                 # TUI surfaces. Only the wiring layer may import these.
    ...
    agent-status.ts   # Running-agent status line and the activity tracker it feeds
  ...
  tools/              # The registered tools. Nothing but index.ts may import these.
    deps.ts             # The slice of the activation context this family needs
  ...
  extension/
    context.ts        # The activation context: the state cells and live handles that
                      # index.ts's clusters share, held behind accessors so the wiring can be
