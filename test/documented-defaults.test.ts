...
//
// The defaults live on the `RunLimits` value the settings appliers overwrite
...
describe("documented defaults (README:441)", () => {
  ...
  // src/agent/run-limits.js sits above the whole pi-coding-agent graph, but a cold
  // re-import under a loaded full run still costs well over the default 5s timeout,
  // so these two get an explicit generous one rather than a retry.
  const HEAVY_REIMPORT_MS = 60_000;

  it("grace turns after the soft limit default to 5", async () => {
    const { getGraceTurns } = await import("../src/agent/run-limits.js");
    expect(getGraceTurns()).toBe(5);
  }, HEAVY_REIMPORT_MS);

  it("max turns is unlimited by default", async () => {
    const { getDefaultMaxTurns } = await import("../src/agent/run-limits.js");
