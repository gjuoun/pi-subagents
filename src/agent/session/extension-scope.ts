/**
 * extension-scope.ts — extension identity and tool-scope resolution for a live session.
 *
 * Moved out of `agent-runner.ts` as-is: 276 lines that decide which extensions a subagent's
 * `extensions:` frontmatter admits, and keep the session's active tool set narrowed to them.
 * Nothing here depends on the rest of the runner, so it moves whole.
 *
 * The session-bound half (`installExtensionToolScope`) installs a `turn_end` listener and a
 * `beforeToolCall` guard that deliberately outlive the call — pi clears them on `dispose()`.
 */
