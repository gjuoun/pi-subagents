 ...
 *
 * Spawned with `new Worker(WORKER_SOURCE, { eval: true })`, so the source is an
 * inlined string: `AGENTS.md` forbids dynamic `import()`, and a file path would
 * have to survive bundling.
 *
 ...
 * runaway script mid-`await`, which an in-process `vm` timeout cannot); the vm
 * context exists for *determinism and accident-avoidance*, not security.
 *
 * The context gets no host built-ins: injecting `Object` would hand the script
 * `constructor` → the host `Function`, a compiler for arbitrary host-realm code.
 * Our injected globals are themselves host closures, so the hygiene shrinks the
 * surface and does not close the hole — **`codeGeneration: { strings: false }` is
 * the load-bearing defense**, since it makes `Function("…")` and `eval("…")` throw
 * `EvalError`. Treat this as a determinism boundary, not a security boundary.
 *
 * Determinism is a prelude rather than a stub: realm built-ins cannot be neutered
 * by injection, so the compiled source is prefixed with code that reassigns
 * `Date.now` and `Math.random` in place and lexically shadows `Date` with a
 * throwing subclass. It is enforced because a workflow's journal is replayed by
 * prefix on resume — a script that reads the clock diverges on the second run.
 */
