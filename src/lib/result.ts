 ...
 *
 * {@link autoTag} turns a plain object of factories into a *catalog*: the object key IS the
 * runtime `code`, written once instead of repeated inside every factory body, and spread
 * in last so a mis-typed payload cannot overwrite it. {@link FactoryUnion} derives the
 * failure union from that catalog, so adding a variant cannot leave the type behind.
 *
 * Errors built this way are plain data (`{ code, message, … }`), never classes: an
 * extension error crosses no HTTP boundary and is consumed in-process by our own code, so
 * it is better off narrowed by `code` than by `instanceof` — and `JSON.stringify`-safe for
 * free. A catalog is declared next to the failure it models (e.g. `model/errors.ts`), not
 * here: one union per failure domain.
 */
